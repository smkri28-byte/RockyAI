...
st.set_page_config(
    page_title="RockyAIv1-9",
    page_icon="🏔️",
    layout="wide",
    initial_sidebar_state="expanded",
)

APP_NAME = "RockyAIv1-9"
...
def v19_extension_prompts():
    """The original RockyAI v1-6 tools, exposed as chat extensions."""
    return {
        "🎓 AI Tutor": "Teach the user's requested topic step by step. Adapt to their level, explain simply, give an example, important points, and practice questions.",
        "🧩 Question Solver": "Solve the user's question step by step. Explain the reasoning clearly and give the final answer. If it is an exam question, keep the final answer suitable for writing in an exam.",
        "📖 PDF Study": "Use the attached PDF/document as the PRIMARY SOURCE. Answer the user's request from the supplied file. Preserve the source's facts and terminology. If the file does not contain the answer, clearly say that.",
        "📄 PDF Generator": "Create polished content for a PDF based on the user's request. Structure it with a title, headings, clear sections and readable formatting. The system will make the generated content downloadable as a PDF.",
        "📝 Quiz Generator": "Create a useful quiz on the requested topic. Include clear questions, suitable difficulty, and an answer key with brief explanations.",
        "📚 Sample Paper": "Create a complete sample question paper for the requested subject/topic. Include sections, marks, suitable difficulty and an answer key.",
        "💻 Code Generator": "Act as RockyAI's coding assistant. Write complete, correct, copy-paste-ready code for the user's request. Explain important parts and fix/debug supplied code when applicable.",
        "🧠 Mind Map": "Create a clear hierarchical text mind map for the requested topic with a central topic, major branches, useful subtopics and key takeaways.",
        "🃏 Flashcards": "Create concise study flashcards for the requested topic. Use clear questions and answers that are useful for revision.",
        "📅 Study Planner": "Create a realistic study plan from the user's request, with subjects/topics, daily tasks, revision, practice and progress checkpoints.",
        "✂️ Smart Summarizer": "Summarize the user's supplied material while preserving important facts, names, definitions and numbers. Do not add facts that are not in the supplied material.",
        "🌐 Translator": "Translate the user's supplied text accurately and naturally into the requested language while preserving meaning, structure and important terminology.",
        "💡 Brainstorm": "Brainstorm practical and creative ideas for the user's topic. Organize the ideas clearly and include useful next steps.",
        "🎯 Exam Preparation": "Create an exam-preparation strategy for the user's request, including topics, revision, practice, time management and a final checklist.",
        "🧪 Periodic Table": "Help with the periodic table using accurate element names, symbols, atomic numbers, groups, periods and useful trends. Answer the user's specific chemistry request clearly.",
        "🏆 Daily Challenge": "Create one engaging daily learning challenge for the requested subject/topic, followed by a hint, answer and short explanation.",
        "🗣️ Debate Coach": "Coach the user for a debate. Provide arguments, counterarguments, evidence prompts, rebuttals and a strong closing statement. Keep it appropriate to the requested level.",
        "🎤 Interview Coach": "Act as an interviewer and coach. Run interview practice based on the user's requested role/goal and give constructive feedback on answers.",
        "🧭 Career Roadmap": "Create a practical career roadmap based only on the user's stated interests, level and goals. Include skills, learning steps, projects and milestones.",
        "🚀 Project Builder": "Turn the user's project idea into requirements, features, architecture, milestones, tasks, technology choices, code/documentation needs and next steps.",
        "📊 Presentation Maker": "Create slide-by-slide presentation content for the user's topic, including titles, concise slide content and speaker notes where useful.",
        "🧠 Memory Trainer": "Create an active-recall memory workout for the user's topic using chunking, mnemonics, retrieval questions and spaced-review suggestions.",
        "🎯 Goal Coach": "Turn the user's goal into measurable milestones, weekly actions, practical next steps and progress checks.",
        "📘 Vocabulary Builder": "Build a vocabulary lesson for the user's topic with useful words, simple meanings, example sentences, memory clues and a short quiz.",
        "🔍 Fact Checker": "Analyze the user's claim carefully. Separate what can be established from what is uncertain. Do not pretend to browse or cite sources that were not accessed.",
    }
...
def v19_extensions_panel():
    ...
    st.markdown(
        "<div class='v19-extension-panel'><div class='v19-extension-title'>RockyAI Tools</div><div class='v19-extension-sub'>Choose one v1-6 tool. It becomes the active mode in the message bar.</div></div>",
        unsafe_allow_html=True,
    )
    cols = st.columns(4)
...
.v19-active-tool span { opacity: .65; }
.v19-active-tool strong { font-size: 15px; }
</style>
""", unsafe_allow_html=True)
    name = st.session_state.v17_active_chat
    messages = st.session_state.v17_chats[name]
    extension_map = v19_extension_prompts()

    st.markdown(
        f"<div class='v19-topbar'><div><div class='v19-brand'>🏔️ RockyAIv1-9</div><div class='v19-sub'>AI powered personal workspace</div></div><div class='v19-status'><span></span> Online</div></div>",
        unsafe_allow_html=True,
    )

    if not messages:
        st.markdown(
            "<div class='v19-welcome'><div class='v19-big-logo'>🏔️</div><div class='v19-kicker'>YOUR PERSONAL AI WORKSPACE</div><h1>What are we building today?</h1><p>Choose a RockyAI tool from +, attach a file if needed, then ask your question.</p></div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f"<div class='v19-chat-title'><span>🏔️</span><strong>{html.escape(name)}</strong><small>RockyAI conversation</small></div>",
            unsafe_allow_html=True,
        )
    ...
    if st.session_state.v19_extensions_open:
        st.markdown(
            "<style>.st-key-v19_plus_wrap button{transform:rotate(45deg)!important;background:linear-gradient(135deg,#ef233c,#b7092b)!important}</style>",
            unsafe_allow_html=True,
        )

    with st.container(key="v19_plus_wrap"):
        if st.button("＋", key="v19_plus_button", help="Open RockyAI tools"):
            st.session_state.v19_extensions_open = not st.session_state.v19_extensions_open
            st.rerun()

    if st.session_state.v19_extensions_open:
        st.markdown(
            "<div class='v19-menu-row'><b>RockyAI Tools</b><span>Select one tool to make it the active message-bar extension</span></div>",
            unsafe_allow_html=True,
        )
    ...
    # The selected extension is displayed directly above the message input,
    # so the user always knows which v1-6 tool will process the message.
    active_extension = st.session_state.get("v19_active_extension")
    if active_extension:
        st.markdown(
            f"""
            <div class='v19-active-tool'>
                <span>Active extension</span>
                <strong>{html.escape(active_extension)}</strong>
            </div>
            """,
            unsafe_allow_html=True,
        )
        if st.button("✕ Clear tool", key="v19_clear_extension", use_container_width=False):
            v19_clear_extension()

    input_placeholder = f"{active_extension}  •  Message RockyAI…" if active_extension else "Message RockyAI…"
    prompt = st.chat_input(input_placeholder, key="v19_input")
    ...
    if prompt and prompt.strip():
        ...
        if active_extension and active_extension in extension_map:
            tool_instruction = extension_map[active_extension]
            tool_name = active_extension
        else:
            tool_instruction = "Answer the user's request as the normal RockyAI assistant."
            tool_name = "Ask RockyAI"

        attachment_text, _ = attachment_context()
        attachment_note = ""
        if attachment_text:
            attachment_note = "\nA file is attached. Use it when relevant, especially for PDF Study, Summarizer, Question Solver and file/code questions."

        instruction = f"""
You are operating in RockyAI v1-9 using the selected v1-6 tool: {tool_name}.

TOOL BEHAVIOR:
{tool_instruction}

IMPORTANT:
- This selected tool is the mode for this message. Do not switch to a different tool unless the user explicitly asks.
- Preserve the useful behavior of the original RockyAI v1-6 tools.
- Give a direct useful result, not a description of what you could do.
- If the selected tool needs information that the user did not provide, ask only for the missing information.
- For attached files, use the supplied file as the primary source when the selected tool concerns that file.{attachment_note}
- Image generation is not supported.
"""

        with st.spinner(f"{active_extension or 'RockyAI'} is working…"):
            answer = ask_rockyai(prompt, instruction)

        messages.append(("assistant", answer))
        save_chat(tool_name, prompt, answer)

        # Keep the selected extension active for the next message, like a real mode.
        # The user can clear it with the X button.
        st.rerun()
...
st.markdown('<div class="rocky-footer">RockyAIv1-9 • Learn • Practice • Create • Build</div>', unsafe_allow_html=True)
