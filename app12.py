...
# ============================================================
# ROCKYAI v1-7 — PAGE CONFIG
# ============================================================

st.set_page_config(
    page_title="RockyAIv1-7",
    page_icon="🏔️",
    layout="wide",
...
def ask_rockyai(prompt, instruction=""):
    ...
    final_prompt = f"""
You are RockyAIv1-7, a professional educational AI assistant.
...
def login_page():
    ...
    st.markdown(
        '<div class="rocky-footer">RockyAIv1-7 • AI-powered learning workspace</div>',
        unsafe_allow_html=True,
    )
...
def v17_chat_ui():
    ...
    if not messages:
        st.markdown('''<div class="v17-welcome"><div class="v17-logo">🏔️</div><h1>How can I help you today?</h1><p>Start a new conversation with RockyAI. Attach files whenever you need.</p></div>''', unsafe_allow_html=True)
        a,b,c=st.columns(3)
        for col,label in [(a,"🎓 Explain a topic"),(b,"🧩 Solve a question"),(c,"🚀 Build a project")]:
            with col:
                if st.button(label,use_container_width=True,key="v17_suggest_"+label):
                    st.session_state.v17_prefill=label.split(" ",1)[1]; st.rerun()
    ...
    attachment_uploader()
    prompt=st.chat_input("Message RockyAI...", key="v17_input")
    if prompt and prompt.strip():
        prompt=prompt.strip(); messages.append(("user",prompt))
        ...
        with st.spinner("RockyAI is thinking..."):
            answer=ask_rockyai(prompt)
        messages.append(("assistant",answer)); save_chat("New Chat",prompt,answer); st.rerun()
...
def workspace():
    v17_chat_ui()
    st.divider()
    st.markdown("### 🧰 All RockyAI Tools")
    if "active_tool" not in st.session_state:
        st.session_state.active_tool = TOOLS[0]
    selected = st.selectbox("Choose a tool", TOOLS, index=TOOLS.index(st.session_state.active_tool), key="tool_selector")
    st.session_state.active_tool = selected
    functions = {
        "🤖 Ask RockyAI": ask_tool, "🎓 AI Tutor": tutor_tool, "🧩 Question Solver": solver_tool, "📖 PDF Study": pdf_study_tool,
        "📄 PDF Generator": pdf_generator_tool, "📝 Quiz Generator": quiz_tool, "📚 Sample Paper": sample_paper_tool, "💻 Code Generator": code_tool,
        "🧠 Mind Map": mindmap_tool, "🃏 Flashcards": flashcards_tool, "📅 Study Planner": planner_tool, "✂️ Smart Summarizer": summarizer_tool,
        "🌐 Translator": translator_tool, "💡 Brainstorm": brainstorm_tool, "🎯 Exam Preparation": exam_tool, "🧪 Periodic Table": periodic_tool,
        "🏆 Daily Challenge": daily_challenge_tool, "🗣️ Debate Coach": debate_tool, "🎤 Interview Coach": interview_tool, "🧭 Career Roadmap": career_tool,
        "🚀 Project Builder": project_tool, "📊 Presentation Maker": presentation_tool, "🧠 Memory Trainer": memory_tool, "🎯 Goal Coach": goal_tool,
        "📘 Vocabulary Builder": vocabulary_tool, "🔍 Fact Checker": fact_checker_tool,
    }
    functions[selected]()
...
st.markdown(
    '<div class="rocky-footer">RockyAIv1-7 • Learn • Practice • Create • Build</div>',
    unsafe_allow_html=True,
)
